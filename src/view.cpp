...
View::View() {
	...
	auto make_str_buf = [] {
		return std::vector<std::vector<std::string>>(
		    GAME_WINDOW_HEIGHT,
		    std::vector<std::string>(GAME_WINDOW_WIDTH, ""));
	};
	auto make_col_buf = [] {
		return std::vector<std::vector<Color>>(
		    GAME_WINDOW_HEIGHT,
		    std::vector<Color>(GAME_WINDOW_WIDTH, Color::NOCHANGE));
	};
...
void View::render() {
	...
	bool dirty = (last_map != latest_map || last_fg_color != latest_fg_color ||
	              last_bg_color != latest_bg_color);
	if ( !dirty )
		return;

	std::string frame;
	frame.reserve((GAME_WINDOW_HEIGHT + 2) *
	              (GAME_WINDOW_CELL_WIDTH * GAME_WINDOW_WIDTH + 3));
...
void View::resetLatest() {
	for ( int r = 0; r < GAME_WINDOW_HEIGHT; ++r ) {
		...
		std::fill(latest_fg_color[r].begin(), latest_fg_color[r].end(),
		          Color::NOCHANGE);
		std::fill(latest_bg_color[r].begin(), latest_bg_color[r].end(),
		          Color::NOCHANGE);
	}
}
